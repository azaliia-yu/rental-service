...
if (offerContent) {
  ...
  console.warn(
    'Не найден /offers POST requestBody content multipart/form-data — пример не добавлен'
  );
}

fs.writeFileSync(SWAGGER_PATH, YAML.stringify(doc), 'utf-8');
console.log('Готово! Примеры для POST /login и POST /offers записаны в', SWAGGER_PATH);